...
for(gene in colnames(day115_125_gene_expression_data)[colnames(day115_125_gene_expression_data) != "Main_cluster_name"]){
  ...
  #creating a dataframe with percent and logodds columns corresponding to probability that cell is an Microglia depending on counts for that gene
  ...
  for(n in unique(count_df$count)){
    ...
    # count for all Microglia
    n_Microglia <- sum(day115_125_gene_expression_data[day115_125_gene_expression_data$Main_cluster_name == "Microglia",gene]==n)
    
    # probability of a cell being an Microglia
    # number of Microglia w unique count value for spp1 / all cells
...
Microglia.markers <- c("S100B","GFAP", "APOE", "AQP4")
Microglia.markers <- Microglia.markers[!Microglia.markers %in% topgenes_logodds]
topgenes_logodds <- append(topgenes_logodds, Microglia.markers)
...
#creating binary predcictor column (Microglia = 1, other = 0)
...
#testing with day110 Microglia model
...
#original odds = number of Microglia / number of not Microglia in training data
#undersample odds <- number of Microglia / number of not Microglia in undersampled data
...
#save the nde
saveRDS(rf, file = "/Users/saanviiyer/Documents/GitHub/SCRI_r/RF/Microgliarf.RDS")
