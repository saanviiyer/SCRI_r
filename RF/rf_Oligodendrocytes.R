...
seuobj_115_125 <- list("115" = seuobj115,
                       "125" = seuobj125)

#identifying features
features <- SelectIntegrationFeatures(object.list = seuobj_115_125)
#creating anchors
subsets.anchors <- FindIntegrationAnchors(object.list = seuobj_115_125, anchor.features = features)
#creating integrated obj
seuobj_115_125 <- IntegrateData(anchorset = subsets.anchors)

seuobj_115_125 <- NormalizeData(seuobj_115_125)
seuobj_115_125 <- FindVariableFeatures(seuobj_115_125, selection.method = "vst", nfeatures = 2000)
#Scaling-
all.genes <- rownames(seuobj_115_125)
seuobj_115_125 <- ScaleData(seuobj_115_125, features = all.genes)
#PCA
...
for(gene in colnames(day115_125_gene_expression_data)[colnames(day115_125_gene_expression_data) != "Main_cluster_name"]){
  ...
  #creating a dataframe with percent and logodds columns corresponding to probability that cell is an Oligodendrocyte depending on counts for that gene
  ...
  for(n in unique(count_df$count)){
    ...
    # probability of a cell being an Oligodendrocyte
...
Oligodendrocyte.markers <- c("S100B","GFAP", "APOE", "AQP4")
Oligodendrocyte.markers <- Oligodendrocyte.markers[!Oligodendrocyte.markers %in% topgenes_logodds]
topgenes_logodds <- append(topgenes_logodds, Oligodendrocyte.markers)
...
#creating binary predcictor column (Oligodendrocyte = 1, other = 0)
...
#testing with day110 Oligodendrocyte model
...
#save the nde
saveRDS(model_lm, file = "/Users/saanviiyer/Documents/GitHub/SCRI_r/RF/Oligodendrocyterf.RDS")
