...
for(gene in colnames(day115_125_gene_expression_data)[colnames(day115_125_gene_expression_data) != "Main_cluster_name"]){
  ...
  #creating a dataframe with percent and logodds columns corresponding to probability that cell is an Unipolar_brush_cell depending on counts for that gene
  ...
  for(n in unique(count_df$count)){
    ...
    # count for all Unipolar_brush_cells
    ...
    # probability of a cell being an Unipolar_brush_cell
    # number of Unipolar_brush_cells w unique count value for spp1 / all cells
...
Unipolar_brush_cell.markers <- c("S100B","GFAP", "APOE", "AQP4")
Unipolar_brush_cell.markers <- Unipolar_brush_cell.markers[!Unipolar_brush_cell.markers %in% topgenes_logodds]
topgenes_logodds <- append(topgenes_logodds, Unipolar_brush_cell.markers)
...
#creating binary predcictor column (Unipolar_brush_cell = 1, other = 0)
...
#testing with day110 Unipolar_brush_cell model
...
#original odds = number of Unipolar_brush_cells / number of not Unipolar_brush_cells in training data
#undersample odds <- number of Unipolar_brush_cells / number of not Unipolar_brush_cells in undersampled data
...
#save the nde
saveRDS(rf, file = "/Users/saanviiyer/Documents/GitHub/SCRI_r/RF/Unipolar_brush_cellrf.RDS")
