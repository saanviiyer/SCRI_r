...
for(gene in colnames(day115_125_gene_expression_data)[colnames(day115_125_gene_expression_data) != "Main_cluster_name"]){
  ...
  #creating a dataframe with percent and logodds columns corresponding to probability that cell is an Vascular_endothelial_cell depending on counts for that gene
  ...
  for(n in unique(count_df$count)){
    ...
    # count for all Vascular_endothelial_cells
    ...
    # probability of a cell being an Vascular_endothelial_cell
    # number of Vascular_endothelial_cells w unique count value for spp1 / all cells
...
#Vascular_endothelial_cell model
...
Vascular_endothelial_cell.markers <- c("S100B","GFAP", "APOE", "AQP4")
Vascular_endothelial_cell.markers <- Vascular_endothelial_cell.markers[!Vascular_endothelial_cell.markers %in% topgenes_logodds]
topgenes_logodds <- append(topgenes_logodds, Vascular_endothelial_cell.markers)
...
#creating binary predcictor column (Vascular_endothelial_cell = 1, other = 0)
...
#testing with day110 Vascular_endothelial_cell model
...
#original odds = number of Vascular_endothelial_cells / number of not Vascular_endothelial_cells in training data
#undersample odds <- number of Vascular_endothelial_cells / number of not Vascular_endothelial_cells in undersampled data
...
#save the nde
saveRDS(rf, file = "/Users/saanviiyer/Documents/GitHub/SCRI_r/RF/Vascular_endothelial_cellrf.RDS")
