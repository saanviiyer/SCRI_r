...
for(gene in colnames(day115_125_gene_expression_data)[colnames(day115_125_gene_expression_data) != "Main_cluster_name"]){
  ...
  #creating a dataframe with percent and logodds columns corresponding to probability that cell is an Granule_neuron depending on counts for that gene
  ...
  for(n in unique(count_df$count)){
    ...
    # probability of a cell being an Granule_neuron
...
Granule_neuron.markers <- c("S100B","GFAP", "APOE", "AQP4")
Granule_neuron.markers <- Granule_neuron.markers[!Granule_neuron.markers %in% topgenes_logodds]
topgenes_logodds <- append(topgenes_logodds, Granule_neuron.markers)
...
#creating binary predcictor column (Granule_neuron = 1, other = 0)
...
#testing with day110 Granule_neuron model
