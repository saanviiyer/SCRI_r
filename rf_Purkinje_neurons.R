...
for(gene in colnames(day115_125_gene_expression_data)[colnames(day115_125_gene_expression_data) != "Main_cluster_name"]){
  ...
  #creating a dataframe with percent and logodds columns corresponding to probability that cell is an Purkinje_neuron depending on counts for that gene
  ...
  for(n in unique(count_df$count)){
    ...
    # count for all Purkinje_neurons
    ...
    # probability of a cell being an Purkinje_neuron
    # number of Purkinje_neurons w unique count value for spp1 / all cells
...
Purkinje_neuron.markers <- c("S100B","GFAP", "APOE", "AQP4")
Purkinje_neuron.markers <- Purkinje_neuron.markers[!Purkinje_neuron.markers %in% topgenes_logodds]
topgenes_logodds <- append(topgenes_logodds, Purkinje_neuron.markers)
...
#creating binary predcictor column (Purkinje_neuron = 1, other = 0)
...
#testing with day110 Purkinje_neuron model
...
#original odds = number of Purkinje_neurons / number of not Purkinje_neurons in training data
#undersample odds <- number of Purkinje_neurons / number of not Purkinje_neurons in undersampled data
...
#save the nde
saveRDS(rf, file = "/Users/saanviiyer/Documents/GitHub/SCRI_r/RF/Purkinje_neuronrf.RDS")
