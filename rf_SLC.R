...
for(gene in colnames(day115_125_gene_expression_data)[colnames(day115_125_gene_expression_data) != "Main_cluster_name"]){
  ...
  #creating a dataframe with percent and logodds columns corresponding to probability that cell is an SLC24A4_PEX5L_positive_cell depending on counts for that gene
  ...
  for(n in unique(count_df$count)){
    ...
    # count for all SLC24A4_PEX5L_positive_cells
    ...
    # probability of a cell being an SLC24A4_PEX5L_positive_cell
    # number of SLC24A4_PEX5L_positive_cells w unique count value for spp1 / all cells
...
#SLC24A4_PEX5L_positive_cell model
...
SLC24A4_PEX5L_positive_cell.markers <- c("S100B","GFAP", "APOE", "AQP4")
SLC24A4_PEX5L_positive_cell.markers <- SLC24A4_PEX5L_positive_cell.markers[!SLC24A4_PEX5L_positive_cell.markers %in% topgenes_logodds]
topgenes_logodds <- append(topgenes_logodds, SLC24A4_PEX5L_positive_cell.markers)
...
#creating binary predcictor column (SLC24A4_PEX5L_positive_cell = 1, other = 0)
...
rf <- randomForest(celltype ~. , data = training.data.topgenes, family = "binomial")

#testing GLM with 0.25 GE data
predictions <- rf %>% predict(testing.data.topgenes)
...
# step.model.forward <- stepAIC(rf, direction = "forward", trace = 5)
# summary(step.model.forward)
# 
# step.model.backward <- stepAIC(rf, direction = "backward", trace = 5)
# summary(step.model.backward)
# 
# step.model.both <- stepAIC(rf, direction = "both", trace = 5)
...
#testing with day110 SLC24A4_PEX5L_positive_cell model
...
# Making predictions
probabilities <- rf %>% predict(day110_gene_expression_data, type = "response") 
...
#original odds = number of SLC24A4_PEX5L_positive_cells / number of not SLC24A4_PEX5L_positive_cells in training data
#undersample odds <- number of SLC24A4_PEX5L_positive_cells / number of not SLC24A4_PEX5L_positive_cells in undersampled data
...
#save the nde
saveRDS(rf, file = "/Users/saanviiyer/Documents/GitHub/SCRI_r/RF/SLC24A4_PEX5L_positive_cell_rf.RDS")

#setting tuning metrics to the one for the regular model
tuning_metrics_df_rf <- tuning_metrics_df

#switching from wide to long dataframe
df <- tuning_metrics_df_rf %>% gather(metric, value, f_measure:recall)
